...
class UINewGoal:
    def __init__(self):
        ...
        self.label = uiw.uiLabel(
            master=self.top,
            text="Provide a unique ID and type for the new gstate."
        )
        ...
        self.type_label = uiw.uiLabel(master=self.top, text="Type")
    ...
    def get_result(self):
        return {"id": self.gstate_id, "type": self.gstate_type, "msg": ""}
...
class UIGetObjects:
    def __init__(self, ldr, selectmode="single"):
        ...
        self.label = uiw.uiLabel(
            master=self.top,
            text="Select one or more objects to add to the goal."
        )
        ...
        for obj in obj_data.values():
            obj_entries.append(f"{obj['id']}:{obj['name']}")

        self.listbox_var = tk.StringVar()
        self.listbox = uiw.uiListBox(
            master=self.top,
...
class UIGetItems:
    def __init__(self, ldr, selectmode="single"):
        ...
        self.label = uiw.uiLabel(
            master=self.top,
            text="Select one or more items to add to the goal."
        )
        self.label.grid(row=0, column=0)
        ...
        for itm in item_data.values():
            item_entries.append(f"{itm['id']}:{itm['name']}")
...
class UIGStateConfig(tk.Frame):
    ...
    def build_ui(self):
        ...
        self.goal_selection_frame = uiw.uiLabelFrame(
            master=self.main_frame, text="Goal List"
        )
        self.button_row = uiw.uiQuietFrame(master=self.main_frame)
        self.title_label = uiw.uiLabel(
            master=self.main_frame, text="Goal Config"
        )
        ...
        self.goal_general_frame = uiw.uiLabelFrame(
            master=self.main_frame, text="General Data"
        )
        self.goal_items_touch_frame = uiw.uiLabelFrame(
            master=self.main_frame, text="ITEMS_TOUCH Data"
        )
        self.goal_obj_items_touch_frame = uiw.uiLabelFrame(
            master=self.main_frame, text="OBJ_ITEMS_TOUCH Data"
        )
        self.goal_n_objs_destroyed_frame = uiw.uiLabelFrame(
            master=self.main_frame, text="N_OBJS_DESTROYED Data"
        )
        self.goal_n_objs_in_locs_frame = uiw.uiLabelFrame(
            master=self.main_frame, text="N_OBJS_IN_LOCS Data"
        )
        ...
        self.goal_general_frame.pack(side=tk.LEFT, fill="y")

        # Goal type selection
        self.select_goal_type_combo = uiw.uiComboBox(
        ...
        self.select_goal_listbox = uiw.uiListBox(
            master=self.goal_selection_frame,
            listvariable=self.select_goal_listbox_var,
            selectmode="single",
        )
        self.select_goal_listbox.pack(side=tk.LEFT, fill=tk.BOTH)
        self.select_goal_listbox.bind("<<ListboxSelect>>", self.show_goal)
        ...
        self.delete_goal_button = uiw.uiCarefulButton(
            master=self.button_row,
    ...
    def build_general_goal_ui(self):
        self.goal_id_label = uiw.uiLabel(
            master=self.goal_general_frame,
            text="ID"
        )
        self.goal_id_entry = uiw.EntryHelp(
            master=self.goal_general_frame, text="Unique ID of the goal."
        )
        self.goal_type_label = uiw.uiLabel(
            master=self.goal_general_frame, text="Type"
        )
        self.goal_type_entry = uiw.EntryHelp(
            master=self.goal_general_frame,
            text="The type of the goal. This can only be specified when a new "
            + "goal is created. If you want to change a goal's type, you will "
            + "need to delete the goal and recreate it.",
        )
        self.goal_type_entry.entry.config(state="disabled")
        self.goal_msg_label = uiw.uiLabel(
            master=self.goal_general_frame, text="Desc"
        )
        self.goal_msg_textbox = uiw.uiTextbox(master=self.goal_general_frame)
    ...
    def build_items_touch_ui(self):
        ...
        self.IT_items_listbox.grid(
            row=1, column=0, columnspan=2, sticky="nsew"
        )
        self.IT_add_items_button.grid(
            row=2, column=0, columnspan=2, sticky="ew"
        )
        self.IT_remove_items_button.grid(
            row=3, column=0, columnspan=2, sticky="ew"
        )

    def build_obj_items_touch_ui(self):
        self.OIT_items_label = uiw.uiLabel(
            master=self.goal_obj_items_touch_frame, text="Items"
        )

        self.OIT_items_listbox_var = tk.StringVar()
        self.OIT_items_listbox = uiw.uiListBox(
            master=self.goal_obj_items_touch_frame,
            listvariable=self.OIT_items_listbox_var,
            selectmode="multiple",
        )

        self.OIT_add_items_button = uiw.uiButton(
            master=self.goal_obj_items_touch_frame,
            command=self.OIT_add_items,
            text="Add Item(s)",
        )
        self.OIT_remove_items_button = uiw.uiButton(
            master=self.goal_obj_items_touch_frame,
            command=self.OIT_remove_items,
            text="Remove Item(s)",
        )

        self.OIT_obj_label = uiw.uiLabel(
            master=self.goal_obj_items_touch_frame, text="Object"
        )

        self.OIT_obj_entry = uiw.EntryHelp(
            master=self.goal_obj_items_touch_frame,
            text="The goal is complete when this object is in the same "
            + "location as all items in the list above.\n\nNOTE: The value "
            + "in this field can only be changed via the add/remove object "
            + "buttons below.",
        )
        self.OIT_obj_entry.entry.config(state="disabled")

        self.OIT_add_objs_button = uiw.uiButton(
            master=self.goal_obj_items_touch_frame,
            command=self.OIT_add_object,
            text="Add Object(s)",
        )
        self.OIT_remove_objs_button = uiw.uiButton(
            master=self.goal_obj_items_touch_frame,
        ...
        self.OIT_items_listbox.grid(
            row=1, column=0, columnspan=2, sticky="nsew"
        )
        self.OIT_add_items_button.grid(
            row=2, column=0, columnspan=2, sticky="ew"
        )
        self.OIT_remove_items_button.grid(
            row=3, column=0, columnspan=2, sticky="ew"
        )
        self.OIT_obj_label.grid(row=4, column=0, sticky="ew")
        self.OIT_obj_entry.frame.grid(row=4, column=1, sticky="ew")
        self.OIT_add_objs_button.grid(
            row=5, column=0, columnspan=2, sticky="ew"
        )
        self.OIT_remove_objs_button.grid(
            row=6, column=0, columnspan=2, sticky="ew"
        )

    def build_n_objs_destroyed_ui(self):
        self.NOD_amount_label = uiw.uiLabel(
            master=self.goal_n_objs_destroyed_frame, text="Amount (N)"
        )
        self.NOD_amount_entry = uiw.EntryHelp(
            master=self.goal_n_objs_destroyed_frame,
            text="The number of the objects below that must be destroyed to "
            + "complete this goal. Amount cannot be set to a value larger "
            + "than the number of objects in the listbox below.",
        )

        self.NOD_objs_label = uiw.uiLabel(
            master=self.goal_n_objs_destroyed_frame, text="Objects"
        )

        self.NOD_objs_listbox_var = tk.StringVar()
        self.NOD_objs_listbox = uiw.uiListBox(
            master=self.goal_n_objs_destroyed_frame,
            listvariable=self.NOD_objs_listbox_var,
            selectmode="multiple",
        )
        self.NOD_add_objs_button = uiw.uiButton(
            master=self.goal_n_objs_destroyed_frame,
            command=self.NOD_add_objects,
            text="Add Object(s)",
        )
        self.NOD_remove_objs_button = uiw.uiButton(
            master=self.goal_n_objs_destroyed_frame,
        ...
        self.NOD_objs_listbox.grid(
            row=2, column=0, columnspan=2, sticky="nsew"
        )
        self.NOD_add_objs_button.grid(
            row=3, column=0, columnspan=2, sticky="ew"
        )
        self.NOD_remove_objs_button.grid(
            row=4, column=0, columnspan=2, sticky="ew"
        )

    def build_n_objs_in_locs_ui(self):
        self.NOIL_amount_label = uiw.uiLabel(
            master=self.goal_n_objs_in_locs_frame, text="Amount (N)"
        )
        self.NOIL_amount_entry = uiw.EntryHelp(
            master=self.goal_n_objs_in_locs_frame,
            text="The number of objects that must be in one of the goal "
            + "locations."
        )
        self.NOIL_objs_label = uiw.uiLabel(
            master=self.goal_n_objs_in_locs_frame, text="Objects"
        )
        self.NOIL_objs_listbox_var = tk.StringVar()
        self.NOIL_objs_listbox = uiw.uiListBox(
            master=self.goal_n_objs_in_locs_frame,
            listvariable=self.NOIL_objs_listbox_var,
            selectmode="multiple",
        )
        self.NOIL_add_objs_button = uiw.uiButton(
            master=self.goal_n_objs_in_locs_frame,
            command=self.NOIL_add_objects,
            text="Add Object(s)",
        )
        self.NOIL_remove_objs_button = uiw.uiButton(
            master=self.goal_n_objs_in_locs_frame,
            command=self.NOIL_remove_objects,
            text="Remove Object(s)",
        )

        self.NOIL_locs_label = uiw.uiLabel(
            master=self.goal_n_objs_in_locs_frame, text="Locations"
        )
        self.NOIL_locs_listbox_var = tk.StringVar()
        self.NOIL_locs_listbox = uiw.uiListBox(
            master=self.goal_n_objs_in_locs_frame,
            listvariable=self.NOIL_locs_listbox_var,
            selectmode="multiple",
        )
        self.NOIL_add_locs_button = uiw.uiButton(
            master=self.goal_n_objs_in_locs_frame,
            command=self.NOIL_add_locations,
            text="Add Locations(s)",
        )
        self.NOIL_remove_locs_button = uiw.uiButton(
            master=self.goal_n_objs_in_locs_frame,
        ...
        self.NOIL_add_objs_button.grid(
            row=3, column=0, columnspan=2, sticky="ew"
        )
        self.NOIL_remove_objs_button.grid(
            row=4, column=0, columnspan=2, sticky="ew"
        )
        ...
        self.NOIL_add_locs_button.grid(
            row=7, column=0, columnspan=2, sticky="ew"
        )
        self.NOIL_remove_locs_button.grid(
    ...
    def show_goal_type_attr_frame(self, goal_type):
        ...
        if self.current_goal_attribute_frame != next_attr_frame:
            if self.current_goal_attribute_frame is not None:
    ...
    def select_goal_with_id(self, _id):
        for index, goal_id in enumerate(
            self.select_goal_listbox.get(0, tk.END)
        ):
            if _id == goal_id:
            ...
            if new_id != selected_goal["id"]:
                if new_id in [x["id"] for x in goal_data]:
                    tk.messagebox.showwarning(
                        "Invalid goal ID",
                        f"WARNING: The goal id {new_id} already exists. "
                        + "Goal IDs must be unique.",
                    )
                elif len(new_id) == 0:
                    tk.messagebox.showwarning(
                        "Invalid goal ID", "Goal IDs cannot be empty."
                    )
                else:
    ...
    def update_items_touch(self, selected_goal):
        ...
        if len(items) < 2:
            tk.messagebox.showwarning(
                "Too Few Items", "This goal requires at least two items."
            )
        else:
    ...
    def update_obj_items_touch(self, selected_goal):
        ...
        if len(items) == 0:
            tk.messagebox.showwarning(
                "Too Few Items", "This goal requires at least one item."
            )
        elif len(obj) == 0:
            tk.messagebox.showwarning(
    ...
    def update_n_objs_destroyed(self, selected_goal):
        ...
        if amount > len(objs):
            tk.messagebox.showwarning(
                "Amount Too Large",
                "The value in the amount field cannot be larger than the "
                + "number of objects in the listbox.",
            )
        elif len(objs) == 0:
            tk.messagebox.showwarning(
                "Too Few Objects", "This goal must have at least one object."
            )
        else:
    ...
    def update_n_objs_in_locs(self, selected_goal):
        ...
        if amount > len(objs):
            tk.messagebox.showwarning(
                "Invalid Amount",
                "The value in amount cannot be larger than the number of "
                + "objects.",
            )
    ...
    def delete_goal(self):
        """
        Deletes the currently selected component from the JSON and component
        dictionary.
        """
    ...
    def NOIL_add_locations(self, event=None):
        new_loc = tk.simpledialog.askstring(
            ...
            + "and y must be integers. They should be valid, reachable "
            + "locations for the intended map (not verified by the system).",
        )
        xy = new_loc.split(",")
