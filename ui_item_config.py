...
class UIItemConfig(tk.Frame):
    ...
    def build_ui(self):
        ...
        self.item_selection_column = uiLabelFrame(master=self.main_frame,text="Items")
        self.items_column = uiLabelFrame(master=self.main_frame, text="Item Info")
        self.button_row = uiQuietFrame(master=self.main_frame)
        self.title_label = uiLabel(master=self.main_frame, text="Item Config")
        ...
        self.select_item_listbox = uiListBox(master=self.item_selection_column, listvariable=self.select_item_listbox_var, selectmode='browse')
        self.select_item_listbox.bind("<<ListboxSelect>>", self.show_item_info)
        self.select_item_listbox.pack(side=tk.LEFT,fill=tk.BOTH)

        # Item Info Widgets
        self.item_id_label = uiLabel(master=self.items_column, text="ID")
        self.item_id_entry = EntryHelp(master=self.items_column, text="The unique ID of the item.")
        self.item_name_label = uiLabel(master=self.items_column, text="Name")
        self.item_name_entry = EntryHelp(master=self.items_column, text="The name of the item.")
        self.item_weight_label = uiLabel(master=self.items_column, text="Weight")
        self.item_weight_entry = EntryHelp(master=self.items_column, text="The weight of the item. Weight (and bulk) are used to determine whether an object (with the correct component) can pick up an item.")
        self.item_bulk_label = uiLabel(master=self.items_column, text="Bulk")
        self.item_bulk_entry = EntryHelp(master=self.items_column, text="The bulk (an abstract measure of side) of the item. Bulk (and weight) are used to determine whether an object (with the correct component) can pick up an item.")
        self.item_sprite_label = uiLabel(master=self.items_column, text="Sprite Filename")
        self.item_sprite_entry = EntryHelp(master=self.items_column, text="The filename of the sprite used to display the item on the map.")
        ...
        self.new_item_button = uiButton(
            master=self.button_row, command=self.create_item, text="Create Item"
        )
        self.new_item_button.pack(side=tk.LEFT)
        self.update_item_button = uiButton(
            master=self.button_row, command=self.update_item, text="Update Item"
        )
        self.update_item_button.pack(side=tk.LEFT)
        self.delete_item_button = uiCarefulButton(
            master=self.button_row, command=self.delete_item, text="Delete Item"
        )
        ...
        self.save_to_json_button = uiCarefulButton(
            master=self.button_row, command=self.save_to_json, text="Save Items to JSON"
        )
    ...
    def create_item(self):
        ...
        good_id = False
        while not good_id:
            item_id = askstring("New Item ID", "Please enter an ID for the new item.")
            if item_id == None:
                break
            elif len(item_id) == 0:
                ...
                messagebox.showwarning(
                    "Invalid Item ID", "This ID already exists, please enter a new ID."
                )
            else:
                good_id = True
