...
class UISettings(tk.Toplevel):
    ...
    def BuildUI(self):
        ...
        self.componentsUpdateButton = uiButton(
            master=self.componentsColumn, command="", text="Update"
        )
        ...
        self.objectsUpdateButton = uiButton(
            master=self.objectsColumn, command="", text="Update"
        )
        ...
        self.mapsUpdateButton = uiButton(
            master=self.mapsColumn, command="", text="Update"
        )
    ...
    def initEntryWidgets(self):
        ...
        self.callsignEntry.insert(
            0, self.teamData[self.teamNames[0]]["agent_defs"][0]["callsign"]
        )
        self.squadEntry.insert(
            0, self.teamData[self.teamNames[0]]["agent_defs"][0]["squad"]
        )
        self.agentObjectEntry.insert(
            0, self.teamData[self.teamNames[0]]["agent_defs"][0]["object"]
        )
        self.aiFileEntry.insert(
            0, self.teamData[self.teamNames[0]]["agent_defs"][0]["AI_file"]
        )
        ...
        self.selectComponentCombo.bind(
            "<<ComboboxSelected>>", self.updateComponentsEntryWidgets
        )
        self.componentsIDEntry.insert(0, self.componentIDs[0])
        self.componentsNameEntry.insert(
            0, self.componentData[self.componentIDs[0]].getData("name")
        )
        self.componentsTypeCombo.configure(values=self.componentTypes)
        self.componentsTypeCombo.set(
            self.componentData[self.componentIDs[0]].getData("ctype")
        )
        self.componentsTypeLabel.config(
            text=self.componentData[self.componentIDs[0]].getData("ctype")
        )
        ...
        if self.componentData[self.componentIDs[0]].getData("ctype") == "CnC":
            ...
            self.componentsTypeAttr1Entry.insert(
                0,
                self.componentData[self.componentIDs[0]].getData(
                    self.componentTypeAttr[4]
                ),
            )
        ...
        self.selectObjectsCombo.bind(
            "<<ComboboxSelected>>", self.updateObjectsEntryWidgets
        )
        self.objectsIDEntry.insert(0, self.objectData[self.objectIDs[0]].getData("id"))
        self.objectsNameEntry.insert(
            0, self.objectData[self.objectIDs[0]].getData("name")
        )
        self.objectsFillAliveEntry.insert(
            0, self.objectData[self.objectIDs[0]].getData("fill_alive")
        )
        self.objectsFillDeadEntry.insert(
            0, self.objectData[self.objectIDs[0]].getData("fill_dead")
        )
        self.objectsTextEntry.insert(
            0, self.objectData[self.objectIDs[0]].getData("text")
        )
        self.objectsHealthEntry.insert(
            0, self.objectData[self.objectIDs[0]].getData("health")
        )
        self.objectsDensityEntry.insert(
            0, self.objectData[self.objectIDs[0]].getData("density")
        )
        self.objectsCompIDsEntry.insert(
            0, self.objectData[self.objectIDs[0]].getData("comp_ids")
        )
        self.objectsPointsCountEntry.insert(
            0, self.objectData[self.objectIDs[0]].getData("points_count")
        )
        ...
        self.mapsEdgeObjIDEntry.insert(
            0, self.mapData[self.mapIDs[0]].getData("edge_obj_id")
        )
    ...
    def updateComponentsEntryWidgets(self, v):
        ...
        self.componentsNameEntry.insert(
            0,
            self.componentData[self.componentIDs[currentComponentIdx]].getData("name"),
        )
        self.componentsTypeCombo.configure(values=self.componentTypes)
        self.componentsTypeCombo.set(
            self.componentData[self.componentIDs[currentComponentIdx]].getData("ctype")
        )
        self.componentsTypeLabel.config(
            text=self.componentData[self.componentIDs[currentComponentIdx]].getData(
                "ctype"
            )
        )
        ...
        if (
            ...
            self.componentsTypeAttr1Entry.insert(
                0,
                self.componentData[self.componentIDs[currentComponentIdx]].getData(
                    self.componentTypeAttr[4]
                ),
            )
        elif (
    ...
    def updateObjectsEntryWidgets(self, v):
        ...
        self.objectsIDEntry.insert(
            0, self.objectData[self.objectIDs[currentObjectIdx]].getData("id")
        )
        self.objectsNameEntry.delete(0, tk.END)
        self.objectsNameEntry.insert(
            0, self.objectData[self.objectIDs[currentObjectIdx]].getData("name")
        )
        self.objectsFillAliveEntry.delete(0, tk.END)
        self.objectsFillAliveEntry.insert(
            0, self.objectData[self.objectIDs[currentObjectIdx]].getData("fill_alive")
        )
        self.objectsFillDeadEntry.delete(0, tk.END)
        self.objectsFillDeadEntry.insert(
            0, self.objectData[self.objectIDs[currentObjectIdx]].getData("fill_dead")
        )
        self.objectsTextEntry.delete(0, tk.END)
        self.objectsTextEntry.insert(
            0, self.objectData[self.objectIDs[currentObjectIdx]].getData("text")
        )
        self.objectsHealthEntry.delete(0, tk.END)
        self.objectsHealthEntry.insert(
            0, self.objectData[self.objectIDs[currentObjectIdx]].getData("health")
        )
        self.objectsDensityEntry.delete(0, tk.END)
        self.objectsDensityEntry.insert(
            0, self.objectData[self.objectIDs[currentObjectIdx]].getData("density")
        )
        self.objectsCompIDsEntry.delete(0, tk.END)
        self.objectsCompIDsEntry.insert(
            0, self.objectData[self.objectIDs[currentObjectIdx]].getData("comp_ids")
        )
        self.objectsPointsCountEntry.delete(0, tk.END)
        self.objectsPointsCountEntry.insert(
            0, self.objectData[self.objectIDs[currentObjectIdx]].getData("points_count")
        )
    ...
    def updateMapsEntryWidgets(self, v):
        ...
        self.mapsNameEntry.insert(
            0, self.mapData[self.mapIDs[currentMapIdx]].getData("name")
        )
        self.mapsEdgeObjIDEntry.delete(0, tk.END)
        self.mapsEdgeObjIDEntry.insert(
            0, self.mapData[self.mapIDs[currentMapIdx]].getData("edge_obj_id")
        )
        self.mapsDescEntry.delete(0, tk.END)
        self.mapsDescEntry.insert(
            0, self.mapData[self.mapIDs[currentMapIdx]].getData("desc")
        )
        self.mapsWidthEntry.delete(0, tk.END)
        self.mapsWidthEntry.insert(
            0, self.mapData[self.mapIDs[currentMapIdx]].getData("width")
        )
        self.mapsHeightEntry.delete(0, tk.END)
        self.mapsHeightEntry.insert(
            0, self.mapData[self.mapIDs[currentMapIdx]].getData("height")
        )
    ...
    def updateTeamsJSON(self):
        ...
        self.teamData[self.teamNameEntry.get()]["agent_defs"][0][
            "callsign"
