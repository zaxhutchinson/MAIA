...
class UINewComponent:
    def __init__(self):
        ...
        self.label = uiw.uiLabel(
            master=self.top,
            text="Provide a unique ID, a name and the ctype of the new "
            + "component.",
        )
        self.label.grid(row=0, column=0, columnspan=2)

        self.id_label = uiw.uiLabel(master=self.top, text="ID")
        self.id_label.grid(row=1, column=0)
        self.id_entry = uiw.uiEntry(master=self.top)
        self.id_entry.grid(row=1, column=1)

        self.ctype_label = uiw.uiLabel(master=self.top, text="CType")
        ...
        self.name_label = uiw.uiLabel(master=self.top, text="Name")
        self.name_label.grid(row=3, column=0)
        self.name_entry = uiw.uiEntry(master=self.top)
        self.name_entry.grid(row=3, column=1, padx=50, pady=10)

        self.btn = uiw.uiButton(
            master=self.top, text="Select", command=self.select
        )
...
class UIComponentConfig(tk.Frame):
    ...
    def build_ui(self):
        ...
        self.component_selection_frame = uiw.uiLabelFrame(
            master=self.main_frame, text="Component List"
        )
        self.button_row = uiw.uiQuietFrame(master=self.main_frame)
        self.title_label = uiw.uiLabel(
            master=self.main_frame, text="Component Config"
        )
        ...
        self.comp_general_frame = uiw.uiLabelFrame(
            master=self.main_frame, text="General Data"
        )
        self.comp_fixed_gun_frame = uiw.uiLabelFrame(
            master=self.main_frame, text="FixedGun Data"
        )
        self.comp_engine_frame = uiw.uiLabelFrame(
            master=self.main_frame, text="Engine Data"
        )
        self.comp_radar_frame = uiw.uiLabelFrame(
            master=self.main_frame, text="Radar Data"
        )
        self.comp_cnc_frame = uiw.uiLabelFrame(
            master=self.main_frame, text="CnC Data"
        )
        self.comp_radio_frame = uiw.uiLabelFrame(
            master=self.main_frame, text="Radio Data"
        )
        self.comp_arm_frame = uiw.uiLabelFrame(
            master=self.main_frame, text="Arm Data"
        )
        ...
        self.comp_general_frame.pack(side=tk.LEFT, fill="y")

        # CType selection
        self.select_component_type_combo = uiw.uiComboBox(
            master=self.component_selection_frame
        )
        self.select_component_type_combo.configure(
            state="readonly", values=comp.CTYPES_LIST
        )
        ...
        self.select_comp_listbox = uiw.uiListBox(
            master=self.component_selection_frame,
            listvariable=self.select_comp_listbox_var,
            selectmode="browse",
        )
        self.select_comp_listbox.pack(side=tk.LEFT, fill=tk.BOTH)
        self.select_comp_listbox.bind("<<ListboxSelect>>", self.show_component)
        ...
        # Buttons
        self.components_create_button = uiw.uiButton(
        ...
        self.components_update_button = uiw.uiButton(
            master=self.button_row,
        ...
        self.components_delete_button = uiw.uiCarefulButton(
            master=self.button_row,
            command=self.delete_components,
    ...
    def build_general_comp_ui(self):
        self.components_id_label = uiw.uiLabel(
            master=self.comp_general_frame,
            text="ID:"
        )
        self.components_id_entry = uiw.EntryHelp(
            master=self.comp_general_frame, text="To be added."
        )
        self.components_name_label = uiw.uiLabel(
            master=self.comp_general_frame, text="Name:"
        )
        self.components_name_entry = uiw.EntryHelp(
            master=self.comp_general_frame, text="To be added."
        )
    ...
    def build_fixed_gun_comp_ui(self):
        ...
        self.fg_reload_ticks_label = uiw.uiLabel(
            master=self.comp_fixed_gun_frame, text="Reload Ticks"
        )
        self.fg_reload_ticks_entry = uiw.EntryHelp(
            master=self.comp_fixed_gun_frame,
            text="The number of turns it takes the gun to reload.",
        )
        self.fg_ammo_label = uiw.uiLabel(
            master=self.comp_fixed_gun_frame, text="Ammunition"
        )
        self.fg_ammo_entry = uiw.EntryHelp(
            master=self.comp_fixed_gun_frame, text="The amount of starting "
            + "ammunition."
        )
        self.fg_min_damage_label = uiw.uiLabel(
            master=self.comp_fixed_gun_frame, text="Min Damage"
        )
        self.fg_min_damage_entry = uiw.EntryHelp(
            master=self.comp_fixed_gun_frame,
            text="The minimum damage the weapon causes.",
        )
        self.fg_max_damage_label = uiw.uiLabel(
            master=self.comp_fixed_gun_frame, text="Max Damage"
        )
        self.fg_max_damage_entry = uiw.EntryHelp(
            master=self.comp_fixed_gun_frame,
            text="The maximum damage the weapon causes.",
        )
        self.fg_wpn_range_label = uiw.uiLabel(
            master=self.comp_fixed_gun_frame, text="Range"
        )
        self.fg_wpn_range_entry = uiw.EntryHelp(
            master=self.comp_fixed_gun_frame,
            text="The maximum distance a projectile will travel.",
        )
    ...
    def build_engine_comp_ui(self):
        self.engine_min_speed_label = uiw.uiLabel(
            master=self.comp_engine_frame, text="Min Speed"
        )
        self.engine_min_speed_entry = uiw.EntryHelp(
            master=self.comp_engine_frame,
            text="The minimum speed of the engine. Negative values allow the "
            + "object to move in reverse. If an object cannot reverse "
            + "direction, min speed should be set to 0.0.",
        )
        self.engine_max_speed_label = uiw.uiLabel(
            master=self.comp_engine_frame, text="Max Speed"
        )
        self.engine_max_speed_entry = uiw.EntryHelp(
            master=self.comp_engine_frame,
            text="The maximum speed of the engine. Speed directly translates "
            + "into object velocity."
        )
        self.engine_max_turnrate_label = uiw.uiLabel(
            master=self.comp_engine_frame, text="Max Turnrate"
        )
        self.engine_max_turnrate_entry = uiw.EntryHelp(
            master=self.comp_engine_frame,
            text="The maximum angle an object can rotate in one tick (in "
            + "degrees). Turnrate is symmetric."
        )
    ...
    def build_radar_comp_ui(self):
        self.radar_range_label = uiw.uiLabel(
            master=self.comp_radar_frame, text="Radar Range"
        )
        self.radar_range_entry = uiw.EntryHelp(
            master=self.comp_radar_frame,
            text="The maximum distance the radar can detect objects."
        )
        self.radar_level_label = uiw.uiLabel(
            master=self.comp_radar_frame, text="Radar Level"
        )
        self.radar_level_entry = uiw.EntryHelp(
            master=self.comp_radar_frame,
            text="A higher radar allows the radar beam to pass through "
            + "objects with a lower density. For example, a radar with a "
            + "level of 100 can see through objects of density 0 to 99. "
            + "However, it could not see through an object with a density "
            + "of 100."
        )
        self.radar_visarc_label = uiw.uiLabel(
            master=self.comp_radar_frame, text="Visible Arc"
        )
        self.radar_visarc_entry = uiw.EntryHelp(
            master=self.comp_radar_frame,
            text="A radar's visible arc (and resolution) determine how much "
            + "of the environment the radar can see. A radar's with a visible "
            + "arc extends from its offset angle in both directions. Within "
            + "this arc around the offset, it sends out detection rays. "
            + "How many rays and how far apart the rays are is determined by "
            + "the 'resolution' attribute."
        )
        self.radar_offset_angle_label = uiw.uiLabel(
            master=self.comp_radar_frame, text="Offset Angle"
        )
        self.radar_offset_angle_entry = uiw.EntryHelp(
            master=self.comp_radar_frame,
            text="The offset angle (in degrees) gives the direction the radar "
            + "points relative to the front of the object. For example, an "
            + "offset angle of 0 means the radar faces the same direction as "
            + "the object. An offset of 180 would mean the radar sees behind "
            + "the object."
        )
        self.radar_resolution_label = uiw.uiLabel(
            master=self.comp_radar_frame, text="Resolution"
        )
        self.radar_resolution_entry = uiw.EntryHelp(
            master=self.comp_radar_frame,
            text="The resolution (in degrees) determines the interval "
            + "between radar beams within its visible arc. For example, if a "
            + "radar has an offset of 0, a visible arc of 20 and a resolution "
            + "of 5, the radar will send out beams at the following degrees "
            + "(as offsets from the object's facing): 20, 15, 10, 5, 0, -5, "
            + "-10, -15, -20."
        )
    ...
    def build_cnc_comp_ui(self):
        self.cnc_max_commands_per_tick_label = uiw.uiLabel(
            master=self.comp_cnc_frame, text="Max Cmds per Tick"
        )
        self.cnc_max_commands_per_tick_entry = uiw.EntryHelp(
            master=self.comp_cnc_frame,
            text="The maximum number of commands, across all components, "
            + "that can be processed in a given turn.",
        )
        self.cnc_max_commands_per_tick_label.grid(
            row=0, column=0, sticky="ew"
        )
        self.cnc_max_commands_per_tick_entry.frame.grid(
            row=0, column=1, sticky="ew"
        )

    def build_radio_comp_ui(self):
        self.radio_max_broadcast_range_label = uiw.uiLabel(
            master=self.comp_radio_frame, text="Max Broadcast Range"
        )
        self.radio_max_broadcast_range_entry = uiw.EntryHelp(
            master=self.comp_radio_frame,
            text="The max distance from the sending object a message can "
            + "be received."
        )
        self.radio_max_broadcast_range_label.grid(
            row=0, column=0, sticky="ew"
        )
        self.radio_max_broadcast_range_entry.frame.grid(
            row=0, column=1, sticky="ew"
        )

    def build_arm_comp_ui(self):
        self.arm_max_weight_label = uiw.uiLabel(
            master=self.comp_arm_frame, text="Max Weight"
        )
        self.arm_max_weight_entry = uiw.EntryHelp(
            master=self.comp_arm_frame,
            text="The maximum weight the arm can lift."
        )
        self.arm_max_bulk_label = uiw.uiLabel(
            master=self.comp_arm_frame, text="Max Bulk"
        )
        self.arm_max_bulk_entry = uiw.EntryHelp(
            master=self.comp_arm_frame,
            text="The maximum bulk the arm can manage."
        )
    ...
    def populate_comp_listbox(self, event=None):
        ...
        for _comp in comp_data.values():
            if _comp["ctype"] == _ctype:
                keys.append(f"{_comp['id']}:{_comp['name']}")
    ...
    def show_ctype_attr_frame(self, _ctype):
        ...
        if self.current_comp_attribute_frame != next_attr_frame:
            if self.current_comp_attribute_frame is not None:
    ...
    def show_cnc(self, comp):
        self.cnc_max_commands_per_tick_entry.entry.insert(
            0, comp["max_cmds_per_tick"]
        )
        ...
        if selected_comp is not None:
            ...
            if new_id != selected_comp["id"]:
                if new_id in self.ldr.get_comp_templates().keys():
                    warnings += f"WARNING: The comp id {new_id} already "
                    + "exists. CompIDs must be unique.\n"
                elif len(new_id) == 0:
                    warnings += "WARNING: ID cannot be empty.\n"
                else:
            ...
            if len(new_name) == 0:
                warnings += "WARNING: Name cannot be empty.\n"
            else:
                selected_comp["name"] = new_name

            if len(warnings) > 0:
                tk.messagebox.showwarning("Warning", warnings)
    ...
    def update_fixed_gun(self, selected_comp):
        try:
            selected_comp["reload_ticks"] = int(
                self.fg_reload_ticks_entry.entry.get()
            )
            selected_comp["ammunition"] = int(
                self.fg_ammo_entry.entry.get()
            )
            selected_comp["min_damage"] = int(
                self.fg_min_damage_entry.entry.get()
            )
            selected_comp["max_damage"] = int(
                self.fg_max_damage_entry.entry.get()
            )
            selected_comp["range"] = int(
                self.fg_wpn_range_entry.entry.get()
            )
        except ValueError as e:
            tk.messagebox.showwarning("Warning", str(e))

    def update_engine(self, selected_comp):
        try:
            selected_comp["min_speed"] = float(
                self.engine_min_speed_entry.entry.get()
            )
            selected_comp["max_speed"] = float(
                self.engine_max_speed_entry.entry.get()
            )
            selected_comp["max_turnrate"] = float(
                self.engine_max_turnrate_entry.entry.get()
            )
        except ValueError as e:
            tk.messagebox.showwarning("Warning", str(e))
    ...
    def update_radar(self, selected_comp):
        try:
            ...
            selected_comp["offset_angle"] = int(
                self.radar_offset_angle_entry.entry.get()
            )
            selected_comp["resolution"] = int(
                self.radar_resolution_entry.entry.get()
            )
        except ValueError as e:
            tk.messagebox.showwarning("Warning", str(e))
    ...
    def update_cnc(self, selected_comp):
        try:
            selected_comp["max_cmds_per_tick"] = int(
                self.cnc_max_commands_per_tick_entry.entry.get()
            )
        except ValueError as e:
            tk.messagebox.showwarning("Warning", str(e))
    ...
    def update_radio(self, selected_comp):
        try:
            selected_comp["max_range"] = int(
                self.radio_max_broadcast_range_entry.entry.get()
            )
        except ValueError as e:
            tk.messagebox.showwarning("Warning", str(e))
    ...
    def update_arm(self, selected_comp):
        try:
            selected_comp["max_bulk"] = int(
                self.arm_max_bulk_entry.entry.get()
            )
            selected_comp["max_weight"] = int(
                self.arm_max_weight_entry.entry.get()
            )
        except ValueError as e:
            tk.messagebox.showwarning("Warning", str(e))
    ...
    def create_component(self):
        ...
        while not good:
            ...
            if result["ctype"] in comp.CTYPES_LIST:
                if len(result["id"]) > 0 \
                        and result["id"] not in comp_data.keys():
                    if len(result["name"]) > 0:
                        good = True
                    else:
                        tk.messagebox.showwarning(
                            "Warning",
                            "A comp must have a name. Does not have to be "
                            + "unique."
                        )
                else:
                    tk.messagebox.showwarning(
    ...
    def delete_components(self):
        """
        Deletes the currently selected component from the JSON
